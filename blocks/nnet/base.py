...
from blocks.roles import (add_role, PARAMETER, VariableRole,
                          WEIGHT, BIAS, OUTPUT)
...
class NNConfig(object):
    ...
    def create_params(self, spec, shape, name, roles=[]):
...
class Dense(NNOps):
    ...
    def _transpose(self):
        ...
        if self.b_init is not None:
            config.create_params(self.b_init, shape=(num_units,), name='b', roles=BIAS)
    ...
    def _initialize(self, num_inputs):
        ...
        config.create_params(self.W_init, shape, 'W', WEIGHT)
        if self.b_init is not None:
            config.create_params(self.b_init, (self.num_units,), 'b', BIAS)
        return config
