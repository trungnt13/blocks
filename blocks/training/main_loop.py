...
# Tasks
# ===========================================================================
class _task_description(object):

    @autoinit
    def __init__(self, func, data, epoch, p, read_data):
        super(_task_description, self).__init__()
        if not (inspect.isfunction(func) or
...
class Task(object):
    ...
    def __init__(self, batch_size=256, dataset=None, seed=None, shuffle=True,
                 name=None):
    ...
    def set_task(self, func, data, epoch=1, p=1., read_data=1):
        self._task = _task_description(func, data, epoch, p, read_data)

    def set_subtask(self, func, data, freq=0., epoch=float('inf'),
                    when=0, read_data=1):
        self._subtask.append(
            _task_description(func, data, epoch, p=1., read_data=read_data))

    def set_crosstask(self, func, data, p=0.5, epoch=float('inf'),
                      when=0, read_data=1):
        self._subtask.append(
            _task_description(func, data, epoch, p, read_data))
